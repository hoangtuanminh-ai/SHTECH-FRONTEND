// src/pages/Dashboard/TrangThaiMayThang.jsx
// Trang Báo Cáo Biểu Đồ Trạng Thái SCADA Theo Tháng (Hiển thị đồng thời 18 Máy: 3 Cắt Vải + 15 Thành Hình)
...
import {
  ...
  FaIndustry, FaLayerGroup, FaThLarge, FaFilePdf
} from 'react-icons/fa';
import { exportDashboardToPDF } from '../../utils/exportPdfHelper';

// Import API chính thức
import { getMachineStatusTimesByMonth } from '../../api/thanhhinhApi';
...
/* ─── Component Card hiển thị SCADA Tháng cho 1 Máy (Chuẩn 4 Cột / Hàng) ───── */
const MachineScadaMonthCard = ({ machine, data }) => {
...
const TrangThaiMayThang = () => {
  console.log(">>> [TrangThaiMayThang] Render trang Trạng Thái 18 Máy SCADA Theo Tháng (4 Máy / Hàng)");
  ...
  // 2. Hàm gọi API lấy dữ liệu SCADA theo Tháng đồng thời cho tất cả 18 máy
  const loadAll18MachinesMonthData = useCallback(async (yearVal, monthVal) => {
    setIsLoading(true);
    const ymStr = `${yearVal}${String(monthVal).padStart(2, '0')}`;
    console.log(`>>> [TrangThaiMayThang] Tải dữ liệu SCADA 18 máy cho Tháng=${ymStr}`);

    try {
      const resultsMap = {};

      // Gọi đồng thời API getMachineStatusTimesByMonth cho cả 18 máy
      const machinePromises = full18Machines.map(async (m) => {
        const maMay = m.code; // 'ORC-TH-01', 'ORC-CV-01'

        try {
          const res = await getMachineStatusTimesByMonth({ maMay, yearMonth: ymStr });
          const rawData = Array.isArray(res) ? res : (Array.isArray(res?.data) ? res.data : []);

          // Tổng hợp dữ liệu tháng theo danh mục SCADA chuẩn
          ...
          console.error(`>>> [TrangThaiMayThang] Lỗi tải SCADA tháng cho máy ${maMay}:`, e);
          resultsMap[maMay] = {
            rawData: [],
      ...
      console.log(">>> [TrangThaiMayThang] Hoàn tất tổng hợp SCADA Tháng cho 18 máy:", resultsMap);

      setMachinesData(resultsMap);
      setLastUpdated(dayjs().format('HH:mm:ss DD/MM/YYYY'));
    } catch (error) {
      console.error(">>> [TrangThaiMayThang] Lỗi toàn cục khi tải SCADA Tháng:", error);
    } finally {
      setIsLoading(false);
    }
  }, [full18Machines]);

  // 3. Tự động tải dữ liệu khi thay đổi Năm hoặc Tháng
  useEffect(() => {
    loadAll18MachinesMonthData(selectedYear, selectedMonth);
  }, [selectedYear, selectedMonth, loadAll18MachinesMonthData]);

  // Nút Refresh (Tìm kiếm và tải lại báo cáo SCADA tháng)
  const handleRefresh = () => {
    console.log(">>> [TrangThaiMayThang] Click Refresh báo cáo SCADA tháng:", { selectedYear, selectedMonth });
    loadAll18MachinesMonthData(selectedYear, selectedMonth);
  };
  ...
  // Xử lý xuất báo cáo PDF cho Trạng Thái Máy Tháng
  const handleExportPDF = async () => {
    console.log(">>> [TrangThaiMayThang] Bắt đầu xuất PDF Trạng thái máy theo tháng...", { selectedYear, selectedMonth, activeTab });
    setIsExportingPDF(true);
    try {
      await exportDashboardToPDF({
        element: containerRef.current,
        title: `BÁO CÁO TRẠNG THÁI HOẠT ĐỘNG SCADA THIẾT BỊ THÁNG ${selectedMonth}/${selectedYear}`,
        fileName: `Bao_Cao_Trang_Thai_May_Thang_${selectedMonth}_${selectedYear}`,
        filterInfo: `Tháng: ${selectedMonth}/${selectedYear} | Bộ lọc: ${activeTab === 'ALL' ? 'Toàn bộ 18 máy' : activeTab === 'CV' ? '3 máy Cắt Vải' : '15 máy Thành Hình'}`
      });
      console.log(">>> [TrangThaiMayThang] Xuất PDF hoàn tất.");
    } catch (err) {
      console.error(">>> [TrangThaiMayThang] Lỗi xuất PDF:", err);
    } finally {
    ...
    <div ref={containerRef} className="drc-scada-container mes-fade">
      ...
      <div className="drc-header-panel">
        <div className="drc-title-left">
          ...
          <h1>THEO DÕI TRẠNG THÁI SCADA THÁNG TOÀN BỘ 18 MÁY (3 CV & 15 TH)</h1>
        </div>
        <div className="drc-header-meta">
          <span>Cập nhật lúc: <strong>{lastUpdated || 'Đang tải...'}</strong></span>
          {isLoading && <span style={{ color: '#0284c7', fontWeight: 800 }}>• Đang quét dữ liệu SCADA tháng của 18 máy...</span>}
        </div>
      </div>

      {/* ── 2. FILTER BAR (NĂM, THÁNG & TABS CÔNG ĐOẠN) ── */}
      <div className="drc-filter-bar">
        <div className="drc-filter-group">
          ...
          {/* Chọn Tháng */}
          <div className="drc-field">
            <span className="drc-label">Tháng:</span>
            <select
              className="drc-select"
              value={selectedMonth}
            ...
            >
              {Array.from({ length: 12 }, (_, i) => i + 1).map(m => (
                <option key={`opt-month-${m}`} value={m}>Tháng {String(m).padStart(2, '0')}</option>
              ))}
            </select>
          </div>
          ...
          {/* Nút Tháng này */}
          <button className="drc-btn drc-btn-secondary" onClick={handleResetMonth}>
            <FaRedo /> Tháng này
          </button>
          ...
          <button
            ...
            title="Xuất báo cáo PDF trực quan toàn bộ trạng thái máy theo tháng"
            style={{
              background: '#dc2626',
              borderColor: '#b91c1c',
              color: '#ffffff',
      ...
      {/* ── 3. LƯỚI HIỂN THỊ SCADA 18 MÁY THEO THÁNG (4 MÁY / HÀNG) ── */}

      {/* Phân nhóm Cắt Vải (3 Máy) */}
      {(activeTab === 'ALL' || activeTab === 'CV') && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          <div className="drc-group-head cv">
            <span>CÔNG ĐOẠN CẮT VẢI (CV) - TRẠNG THÁI SCADA THÁNG {String(selectedMonth).padStart(2, '0')}/{selectedYear}</span>
          ...
          <div className="drc-machine-grid-4">
            {full18Machines.filter(m => m.type === 'CV').map(machine => (
              <MachineScadaMonthCard
                key={`card-month-${machine.code}`}
                machine={machine}
      ...
      {(activeTab === 'ALL' || activeTab === 'TH') && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginTop: activeTab === 'ALL' ? '6px' : '0' }}>
          <div className="drc-group-head th">
            <span>CÔNG ĐOẠN THÀNH HÌNH (TH) - TRẠNG THÁI SCADA THÁNG {String(selectedMonth).padStart(2, '0')}/{selectedYear}</span>
...
export default TrangThaiMayThang;
