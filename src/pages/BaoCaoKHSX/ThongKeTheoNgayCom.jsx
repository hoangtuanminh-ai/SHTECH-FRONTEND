...
import { getThongKeLopKsclTheoNgayCom, getViewStoreListOrks, getDmMayXRay, getListToSx } from '../../api/kcsWebApi';
...
const columns = [
  { key: 'tenQuycachLop', header: 'Quy Cách' },
  { key: 'soluong', header: 'Số Lượng (Chiếc)' },
  { key: 'sanluongLoai1', header: 'Loại 1 (Chiếc)' },
  { key: 'sanluongPhetandung', header: 'Loại Phế Tận Dụng (Chiếc)' },
  { key: 'sanluongPhebo', header: 'Loại Phế (Chiếc)' },
  { key: 'loai1Pct', header: 'Loại 1 (%)' },
  { key: 'phePct', header: 'Phế (%)' }
];

const ThongKeTheoNgayCom = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const res = await getThongKeLopKsclTheoNgayCom(payload);
  ...
  const exportToExcel = async () => {
    ...
    data.forEach((row, idx) => {
      ...
      const rowData = {
        stt: idx + 1,
        tenQuycachLop: row.tenQuycachLop || '',
        soluong: row.soluong !== null && row.soluong !== undefined ? row.soluong : '',
        sanluongLoai1: row.sanluongLoai1 !== null && row.sanluongLoai1 !== undefined ? row.sanluongLoai1 : '',
        sanluongPhetandung: row.sanluongPhetandung !== null && row.sanluongPhetandung !== undefined ? row.sanluongPhetandung : '',
        sanluongPhebo: row.sanluongPhebo !== null && row.sanluongPhebo !== undefined ? row.sanluongPhebo : '',
        loai1Pct: l1Pct,
        phePct: pbPct
      };
  ...
  const exportToPDF = () => {
    ...
    // 1. Góc trên bên trái: Trống (Theo đúng thiết kế ảnh mẫu KS.1.4/BH01)
    ...
    // 5. Định nghĩa dữ liệu cho bảng (có ẩn tên quy cách trùng lặp)
    ...
    const tableBody = data.map((row, idx) => {
      ...
      return [
        ...
        formatNum(row.soluong), // SỐ LƯỢNG (Chiếc)
        formatNum(row.sanluongLoai1), // LOẠI 1 (Chiếc)
        formatNum(row.sanluongPhetandung), // LOẠI PHẾ TẬN DỤNG (Chiếc)
        formatNum(row.sanluongPhebo), // LOẠI PHẾ (Chiếc)
        l1Pct, // LOẠI 1 (%)
        pbPct // PHẾ (%)
      ];
    });
    ...
    const grandTotalL1Pct = grandTotalSL > 0 ? (grandTotalLoai1 / grandTotalSL * 100).toFixed(2) + ' %' : '0.00 %';
    const grandTotalPbPct = grandTotalSL > 0 ? ((grandTotalPheTD + grandTotalPhe) / grandTotalSL * 100).toFixed(2) + ' %' : '0.00 %';
    ...
    autoTable(doc, {
      ...
      head: [
        [
          ...
          { content: 'QUY CÁCH', styles: { halign: 'center', valign: 'middle' } },
          { content: 'SỐ LƯỢNG\n(Chiếc)', styles: { halign: 'center', valign: 'middle' } },
          { content: 'LOẠI 1\n(Chiếc)', styles: { halign: 'center', valign: 'middle' } },
          { content: 'LOẠI\nPHẾ TẬN\nDỤNG\n(Chiếc)', styles: { halign: 'center', valign: 'middle' } },
          { content: 'LOẠI\nPHẾ\n(Chiếc)', styles: { halign: 'center', valign: 'middle' } },
      ...
      columnStyles: {
        ...
        1: { cellWidth: 183, halign: 'left' }, // QUY CÁCH
        2: { cellWidth: 55, halign: 'right' }, // SỐ LƯỢNG (Chiếc)
        3: { cellWidth: 50, halign: 'right' }, // LOẠI 1 (Chiếc)
        4: { cellWidth: 75, halign: 'right' }, // LOẠI PHẾ TẬN DỤNG (Chiếc)
        5: { cellWidth: 55, halign: 'right' }, // LOẠI PHẾ (Chiếc)
        6: { cellWidth: 55, halign: 'right' }, // LOẠI 1 (%)
        7: { cellWidth: 50, halign: 'right' } // PHẾ (%)
      },
      didParseCell: (d) => {
    ...
    // 8. Lưu file
    const docName = `Bang_Tong_Hop_San_Luong_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.pdf`;
    doc.save(docName);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#e0eaf4', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
                    ...
                    return (
                      <tr key={rIdx}>
                        ...
                        {columns.map((col, cIdx) => {
                          let val = '';
                          if (col.key === 'tenQuycachLop') {
                            val = row.tenQuycachLop || '';
                            // Ẩn tên quy cách trùng lặp
                            if (rIdx > 0 && data[rIdx - 1].tenQuycachLop === row.tenQuycachLop) {
                              val = '';
                            }
                          } else if (col.key === 'loai1Pct') {
                            val = l1Pct;
                          } else if (col.key === 'phePct') {
                            val = pbPct;
                          } else {
                            val = row[col.key] !== null && row[col.key] !== undefined && row[col.key] !== 0 ? Number(row[col.key]).toLocaleString('vi-VN') : '';
                          }
                          return <td key={cIdx} style={{ textAlign: col.key === 'tenQuycachLop' ? 'left' : 'right' }}>{val}</td>;
                        })}
                      </tr>
                    );
                  })}
      ...
      <div className="mes-statusbar">
        ...
        <span>CÔNG TY CP CAO SU ĐÀ NẴNG · THỐNG KÊ LỐP THEO NGÀY COM</span>
      </div>
    </div>
  );
};

export default ThongKeTheoNgayCom;
