...
import { getBaoCaoLopTraXuLyNhapKhoKeHoachThang, getBaoCaoLopTraXuLyNhapKhoKeHoachThangCom, getViewStoreListOrks, getDmMayXRay, getListToSx } from '../../api/kcsWebApi';
...
const BaoCaoLopTraXuLyNhapKhoKeHoachThang = () => {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(false);
  
  // Params (tuNgay và denNgay đã bị ẩn, truyền giá trị mặc định khi gọi API)
  ...
  const handleSearch = async () => {
    const namThangNgayCaStart = '19000101'; // Giá trị mặc định
    const namThangNgayCaEnd = '21000101';   // Giá trị mặc định
    ...
    try {
      ...
      let res;
      if (loaiCaNhap) {
        res = await getBaoCaoLopTraXuLyNhapKhoKeHoachThang(payload);
      } else {
        res = await getBaoCaoLopTraXuLyNhapKhoKeHoachThangCom(payload);
      }
  ...
  const exportToExcel = async () => {
    ...
    const fileName = `Bao_Cao_Lop_Tra_Xu_Ly_Nhap_Kho_Ke_Hoach_Thang_${new Date().toISOString().slice(0,10).replace(/-/g, '')}.xlsx`;
    saveAs(new Blob([buffer]), fileName);
    toast.success('Đã xuất file Excel thành công');
  };

  const exportToPDF = () => {
    console.log(">>> [Test Lỗi] Bắt đầu xuất PDF với dữ liệu:", data);
    if (data.length === 0) {
    ...
    // Ánh xạ Ca SX + Tổ SX
    let caText = 'Tất cả';
    const toSxClean = toSX ? toSX.replace(/Tổ\s*/i, '') : '';
    doc.setFont('TimesNewRoman', 'normal');
    doc.text(`Ca SX: ${caText}${toSxClean}`, 110, 54, { align: 'center' });
    ...
    // Hiển thị máy X-Quang nếu có chọn máy (ở trang này mã máy là loaiCaNhap)
    let currentY = 96;
    if (loaiCaNhap) {
      const machineText = `Mã Máy X-Quang: ${loaiCaNhap}`;
      doc.setFont('TimesNewRoman', 'normal');
      doc.setFontSize(9.5);
    ...
    // 6. Lưu file PDF
    const docName = `Bao_Cao_Lop_Tra_Xu_Ly_Nhap_Kho_Ke_Hoach_Thang_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.pdf`;
    doc.save(docName);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#e0eaf4', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
      ...
      <div className="mes-filterbar">
        {/* Đã ẩn tham số Từ ngày và Đến ngày */}
      ...
      <div className="mes-statusbar">
        ...
        <span>CÔNG TY CP CAO SU ĐÀ NẴNG · BÁO CÁO LỐP TRẢ XỬ LÝ NHẬP KHO (KẾ HOẠCH THÁNG)</span>
      </div>
    </div>
  );
};

export default BaoCaoLopTraXuLyNhapKhoKeHoachThang;
