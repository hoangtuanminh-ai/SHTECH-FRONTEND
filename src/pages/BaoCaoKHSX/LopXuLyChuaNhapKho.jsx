...
import { getLopXuLyChuaNhapKhoList } from '../../api/kcsWebApi';
...
const LopXuLyChuaNhapKho = () => {
  ...
  const fetchData = async (pageNum = 0) => {
    setLoading(true);
    try {
      const res = await getLopXuLyChuaNhapKhoList({ page: pageNum });
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      const fileName = `Lop_Xu_Ly_Chua_Nhap_Kho_${new Date().toISOString().slice(0,10).replace(/-/g, '')}.xlsx`;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#e0eaf4', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
      ...
      <div className="mes-statusbar">
        ...
        <span>CÔNG TY CP CAO SU ĐÀ NẴNG · LỐP XỬ LÝ CHƯA NHẬP KHO</span>
      </div>
    </div>
  );
};

export default LopXuLyChuaNhapKho;
