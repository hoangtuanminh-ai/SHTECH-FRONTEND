...
import { getBaoCaoLopTraXuLyNhapKho, getBaoCaoLopTraXuLyNhapKhoCom, getViewStoreListOrks, getDmMayXRay, getListToSx } from '../../api/kcsWebApi';
/* ─── MES Desktop CSS ─────────────────────────────────────────────────────── */
const css = `
...
const BaoCaoLopTraXuLyNhapKho = () => {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(false);
  
  // Params (tuKHThang và denKHThang đã bị ẩn, truyền giá trị mặc định khi gọi API)
  ...
  const handleSearch = async () => {
    ...
    const monthlyPlanStart = '190001'; // Giá trị mặc định
    const monthlyPlanEnd = '210001';   // Giá trị mặc định

    if (!namThangNgayCaStart || !namThangNgayCaEnd) {
      toast.warn('Vui lòng chọn đầy đủ thời gian bắt đầu, kết thúc');
      return;
    }
    ...
    try {
      ...
      let res;
      if (loaiCaNhap) {
        res = await getBaoCaoLopTraXuLyNhapKho(payload);
      } else {
        res = await getBaoCaoLopTraXuLyNhapKhoCom(payload);
      }
  ...
  const exportToExcel = async () => {
    ...
    const fileName = `Bao_Cao_Lop_Tra_Xu_Ly_Nhap_Kho_${new Date().toISOString().slice(0,10).replace(/-/g, '')}.xlsx`;
  ...
  const exportToPDF = () => {
    ...
    if (tuNgay) {
      ...
      filterMonthText = `Tháng ${Number(parts[1])} năm ${parts[0]}`;
    } else {
    ...
    // 6. Lưu file PDF
    const docName = `Bao_Cao_Lop_Tra_Xu_Ly_Nhap_Kho_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.pdf`;
    doc.save(docName);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#e0eaf4', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
      ...
      <div className="mes-filterbar">
        ...
        {/* Đã ẩn tham số Từ KH tháng và Đến KH tháng */}
      ...
      <div className="mes-statusbar">
        ...
        <span>CÔNG TY CP CAO SU ĐÀ NẴNG · BÁO CÁO LỐP TRẢ XỬ LÝ NHẬP KHO</span>
      </div>
    </div>
  );
};

export default BaoCaoLopTraXuLyNhapKho;
