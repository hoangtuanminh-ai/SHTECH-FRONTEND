...
import { getLopThanhPhamTemporaryList } from '../../api/kcsWebApi';
...
const LopThanhPhamTemporary = () => {
  ...
  const fetchData = async (pageNum = 0) => {
    setLoading(true);
    try {
      const res = await getLopThanhPhamTemporaryList({ page: pageNum });
  ...
  const exportToExcel = async () => {
    // Ghi log để phục vụ debug và kiểm tra lỗi theo yêu cầu
    ...
    try {
      ...
      const fileName = `Lop_Thanh_Pham_Temp_${new Date().toISOString().slice(0,10).replace(/-/g, '')}.xlsx`;
      saveAs(new Blob([buffer]), fileName);

      // Ghi log xuất file thành công
      console.log("Đã xuất file Excel thành công với tên: " + fileName);
      toast.success('Đã xuất file Excel thành công');
    } catch (error) {
      // Ghi log lỗi chi tiết khi có sự cố
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#e0eaf4', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
      ...
      <div className="mes-statusbar">
        ...
        <span>CÔNG TY CP CAO SU ĐÀ NẴNG · LỐP THÀNH PHẨM TEMPORARY</span>
      </div>
    </div>
  );
};

export default LopThanhPhamTemporary;
