...
import { getBaoCaoKeHoachThangKhongTheoMay, getBaoCaoKeHoachThangTheoNgay, getViewStoreListOrks, getDmMayXRay, getListToSx } from '../../api/kcsWebApi';
/* ─── MES Desktop CSS ─────────────────────────────────────────────────────── */
const css = `
...
const BaoCaoKeHoachThangKhongTheoMay = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      let res;
      if (maMay) {
        res = await getBaoCaoKeHoachThangTheoNgay(payload);
      } else {
        res = await getBaoCaoKeHoachThangKhongTheoMay(payload);
      }
  ...
  const exportToExcel = async () => {
    ...
    const fileName = `Bao_Cao_Ke_Hoach_Thang_Khong_Theo_May_${new Date().toISOString().slice(0,10).replace(/-/g, '')}.xlsx`;
  ...
  const exportToPDF = () => {
    ...
    const tableBody = data.map((row, idx) => {
      ...
      return [
        ...
        getVal(row.soluong), // SỐ LƯỢNG (Chiếc)
        getVal(row.sanluongLoai1), // LOẠI 1 (Chiếc)
        getVal(row.sanluongPhetandung), // LOẠI PHẾ TẬN DỤNG (Chiếc)
        getVal(row.sanluongPhebo), // LOẠI PHẾ (Chiếc)
        l1Pct, // LOẠI 1 (%)
        pbPct // PHẾ (%)
      ];
    });
    ...
    // Tính tỉ lệ % tổng cộng từ các cột tổng
    const grandL1Pct = grandTotalSL > 0 ? ((grandTotalL1 / grandTotalSL) * 100).toFixed(2) + ' %' : '0.00 %';
    const grandPbPct = grandTotalSL > 0 ? (((grandTotalPheTD + grandTotalPhe) / grandTotalSL) * 100).toFixed(2) + ' %' : '0.00 %';

    // Thêm hàng TỔNG CỘNG
    tableBody.push([
      { content: 'TỔNG CỘNG', colSpan: 2, styles: { halign: 'center', fontStyle: 'bold' } },
      { content: fmtNum(grandTotalSL), styles: { fontStyle: 'bold' } },
      ...
      { content: grandL1Pct, styles: { fontStyle: 'bold' } },
      { content: grandPbPct, styles: { fontStyle: 'bold' } }
    ]);
    ...
    autoTable(doc, {
      ...
      head: [
        [
          ...
          { content: 'LOẠI 1\n(Chiếc)', styles: { halign: 'center', valign: 'middle' } },
          { content: 'LOẠI\nPHẾ TẬN\nDỤNG\n(Chiếc)', styles: { halign: 'center', valign: 'middle', fontSize: 8 } },
          { content: 'LOẠI\nPHẾ\n(Chiếc)', styles: { halign: 'center', valign: 'middle' } },
      ...
      columnStyles: {
        ...
        4: { cellWidth: 65, halign: 'right' },  // LOẠI PHẾ TẬN DỤNG (Chiếc)
        5: { cellWidth: 50, halign: 'right' },  // LOẠI PHẾ (Chiếc)
        6: { cellWidth: 50, halign: 'right' },  // LOẠI 1 (%)
        7: { cellWidth: 45, halign: 'right' }   // PHẾ (%)
      },
      didParseCell: (d) => {
    ...
    // 5. Lưu file PDF
    const docName = `Bao_Cao_Ke_Hoach_Thang_Khong_Theo_May_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.pdf`;
    doc.save(docName);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#e0eaf4', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
      ...
      <div className="mes-statusbar">
        ...
        <span>CÔNG TY CP CAO SU ĐÀ NẴNG · BÁO CÁO KẾ HOẠCH THÁNG KHÔNG THEO MÁY</span>
      </div>
    </div>
  );
};

export default BaoCaoKeHoachThangKhongTheoMay;
