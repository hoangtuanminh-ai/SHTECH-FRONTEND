...
import { getBaoCaoKeHoachThangCa, getBaoCaoKeHoachThangCaCom, getViewStoreListOrks, getDmMayXRay, getListToSx } from '../../api/kcsWebApi';
/* ─── MES Desktop CSS ─────────────────────────────────────────────────────── */
const css = `
...
const BaoCaoKeHoachThangCa = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      let res;
      if (maMay) {
        res = await getBaoCaoKeHoachThangCa(payload);
      } else {
        res = await getBaoCaoKeHoachThangCaCom(payload);
      }
  ...
  const exportToExcel = async () => {
    ...
    const fileName = `Bao_Cao_Ke_Hoach_Thang_Ca_${new Date().toISOString().slice(0,10).replace(/-/g, '')}.xlsx`;
  ...
  const exportToPDF = () => {
    ...
    if (tuKHThang) {
      ...
      monthText = `Tháng ${Number(m)} năm ${y}`;
    } else {
      const today = new Date();
      monthText = `Tháng ${today.getMonth() + 1} năm ${today.getFullYear()}`;
    }

    const caNhapPrefix = caNhap ? caNhap : '';
    const filterMonthText = `${caNhapPrefix}${monthText}`;
    ...
    // 6. Lưu file PDF
    const docName = `Bao_Cao_Ke_Hoach_Thang_Ca_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.pdf`;
    doc.save(docName);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#e0eaf4', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
      ...
      <div className="mes-statusbar">
        ...
        <span>CÔNG TY CP CAO SU ĐÀ NẴNG · BÁO CÁO KẾ HOẠCH THÁNG CA</span>
      </div>
    </div>
  );
};

export default BaoCaoKeHoachThangCa;
