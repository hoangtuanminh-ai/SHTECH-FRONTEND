...
import { getBaoCaoTongHopChiTiet, getViewStoreListOrks, getDmMayXRay, getListToSx } from '../../api/kcsWebApi';
...
const columns = [
  ...
  { key: 'lopCoSam', header: 'Lốp Có Săm' },
  ...
  { key: 'phantramSanluongLoai1', header: '% Loại 1' },
  { key: 'phantramSanluongPhebo', header: '% Phế Bỏ' },
  { key: 'phantramSanluongPhetandung', header: '% Phế TD' },
  { key: 'phantramSanluongTraxuly', header: '% Trả XL' }
];

const BaoCaoTongHopChiTiet = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const res = await getBaoCaoTongHopChiTiet(payload);
  ...
  const exportToExcel = async () => {
    ...
    const fileName = `Bao_Cao_Tong_Hop_Chi_Tiet_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.xlsx`;
  ...
  const exportToPDF = () => {
    ...
    // 1. Góc trên bên trái: Trống (Theo thiết kế ảnh mẫu KS.1.4/BH01)

    // 2. Góc trên bên phảI
    doc.setFontSize(8.5);
    doc.text('KS.1.4/BH01', pageWidth - 23.6, 30, { align: 'right' });

    // 3. Tiêu đề chính ở giữa
    doc.setFontSize(11.5);
    doc.setFont('TimesNewRoman', 'bold');
    const titleText = 'BẢNG TỔNG HỢP SẢN LƯỢNG';
    ...
    // 5. Định nghĩa dữ liệu cho bảng (luôn hiển thị đủ tên quy cách)
    const tableBody = data.map((row, idx) => {
      const displayQuyCach = row.tenQuycachLop || '';

      const sl = Number(row.soluong) || 0;
      const l1 = Number(row.sanluongLoai1) || 0;
      const pb = Number(row.sanluongPhebo) || 0;
      const ptd = Number(row.sanluongPhetandung) || 0;
      const tx = Number(row.sanluongTraxuly) || 0;

      const l1Pct = sl > 0 ? (l1 / sl * 100).toFixed(2) : '0.00';
      const pbPct = sl > 0 ? ((ptd + pb) / sl * 100).toFixed(2) : '0.00';

      return [
        idx + 1, // STT
        displayQuyCach, // QUY CÁCH
        formatNum(row.soluong), // TỔNG SL
        formatNum(row.sanluongLoai1), // LOẠI 1
        formatNum(row.sanluongPhebo), // PHẾ BỎ
        formatNum(row.sanluongPhetandung), // PHẾ TD
        formatNum(row.sanluongTraxuly), // XỬ LÝ
        l1Pct, // % LOẠI 1
        pbPct // % PHẾ
      ];
    });
    ...
    const grandTotalSL = data.reduce((sum, r) => sum + (Number(r.soluong) || 0), 0);
    ...
    // Tính tỷ lệ % của hàng tổng bằng trung bình cộng các tỷ lệ % đã làm tròn của các dòng chi tiết
    const totalRowsCount = data.length || 1;
    const sumL1Pct = data.reduce((sum, row) => {
      const sl = Number(row.soluong) || 0;
      const l1 = Number(row.sanluongLoai1) || 0;
      return sum + Number(sl > 0 ? (l1 / sl * 100).toFixed(2) : '0.00');
    }, 0);
    const sumPbPct = data.reduce((sum, row) => {
      const sl = Number(row.soluong) || 0;
      const ptd = Number(row.sanluongPhetandung) || 0;
      const pb = Number(row.sanluongPhebo) || 0;
      return sum + Number(sl > 0 ? ((ptd + pb) / sl * 100).toFixed(2) : '0.00');
    }, 0);

    const grandTotalL1Pct = (sumL1Pct / totalRowsCount).toFixed(2);
    const grandTotalPbPct = (sumPbPct / totalRowsCount).toFixed(2);

    // Thêm hàng "Tổng số :" vào cuối bảng (in đậm)
    tableBody.push([
      { content: 'Tổng số :', colSpan: 2, styles: { halign: 'left', fontStyle: 'bold' } },
      formatNum(grandTotalSL),
    ...
    // Thêm hàng "Tổng" vào cuối bảng (in đậm)
    tableBody.push([
      { content: 'Tổng', colSpan: 2, styles: { halign: 'left', fontStyle: 'bold' } },
      formatNum(grandTotalSL),
      formatNum(grandTotalLoai1),
      formatNum(grandTotalPhe),
      formatNum(grandTotalPheTD),
    ...
    autoTable(doc, {
      ...
      head: [
        [
          { content: 'STT', styles: { halign: 'center', valign: 'middle' } },
          { content: 'Quy cách lốp', styles: { halign: 'center', valign: 'middle' } },
          { content: 'Tổng SL', styles: { halign: 'center', valign: 'middle' } },
          { content: 'Loại 1', styles: { halign: 'center', valign: 'middle' } },
          { content: 'Phế bỏ', styles: { halign: 'center', valign: 'middle' } },
          { content: 'Phế TD', styles: { halign: 'center', valign: 'middle' } },
          { content: 'Xử lý', styles: { halign: 'center', valign: 'middle' } },
          { content: '% loại 1', styles: { halign: 'center', valign: 'middle' } },
          { content: '% phế', styles: { halign: 'center', valign: 'middle' } }
        ]
      ],
      body: tableBody,
      columnStyles: {
        0: { cellWidth: 25, halign: 'center' }, // STT
        1: { cellWidth: 183, halign: 'left' }, // Quy cách lốp
        2: { cellWidth: 55, halign: 'right' }, // Tổng SL
        3: { cellWidth: 50, halign: 'right' }, // Loại 1
        4: { cellWidth: 50, halign: 'right' }, // Phế bỏ
        5: { cellWidth: 50, halign: 'right' }, // Phế TD
        6: { cellWidth: 50, halign: 'right' }, // Xử lý
        7: { cellWidth: 45, halign: 'right' }, // % loại 1
        8: { cellWidth: 40, halign: 'right' } // % phế
      },
      didParseCell: (d) => {
        d.cell.styles.font = 'TimesNewRoman';
        // In đậm cho 2 hàng tổng cộng cuối cùng
        if (d.row.index >= tableBody.length - 2) {
          d.cell.styles.fontStyle = 'bold';
        }
      }
    });

    // 7. Vẽ Footer ký tên (Đà Nẵng, ngày ... tháng ... năm ... và PHÒNG KCS, Người Lập)
    const finalY = doc.lastAutoTable.finalY + 40;
    doc.setFontSize(10.5);
    doc.setFont('TimesNewRoman', 'normal');
    doc.text('Đà Nẵng, ngày      tháng      năm 2026', pageWidth - 120, finalY - 15, { align: 'center' });
    doc.setFont('TimesNewRoman', 'bold');
    doc.text('PHÒNG KCS', 120, finalY, { align: 'center' });
    doc.text('Người Lập', pageWidth - 120, finalY, { align: 'center' });

    // 8. Lưu file
    const docName = `Bao_Cao_Tong_Hop_Chi_Tiet_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.pdf`;
    doc.save(docName);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#e0eaf4', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
      ...
      <div className="mes-filterbar">
        ...
        <div className="mes-fb-group">
          ...
          <input type="text" className="mes-input" style={{ width: 80 }} value={toSX} onChange={e => setToSX(e.target.value)} />
        </div>
                  ...
                  {data.map((row, rIdx) => (
                    <tr key={rIdx}>
                      ...
                      {columns.map((col, cIdx) => {
                        let val = row[col.key];
                        // Nếu là các cột số lượng và %: nếu rỗng, null, undefined thì hiển thị là 0
                        ...
                        // Căn lề số lượng / tỷ lệ % sang bên phải, còn các cột text khác căn trái
                        const isNumCol = ['sanluongTongBaogomLopTraXuly', 'soluong', 'sanluongLoai1', 'sanluongPhetandung', 'sanluongPhebo', 'sanluongTraxuly', 'phantramSanluongLoai1', 'phantramSanluongPhebo', 'phantramSanluongPhetandung', 'phantramSanluongTraxuly'].includes(col.key);
                        
                        return (
                          <td key={cIdx} style={{ textAlign: isNumCol ? 'right' : 'left' }}>{val}</td>
                        );
                      })}
                    </tr>
                  ))}
      ...
      <div className="mes-statusbar">
        ...
        <span>CÔNG TY CP CAO SU ĐÀ NẴNG · BÁO CÁO TỔNG HỢP CHI TIẾT</span>
      </div>
    </div>
  );
};

export default BaoCaoTongHopChiTiet;
