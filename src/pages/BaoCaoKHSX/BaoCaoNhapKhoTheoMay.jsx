// src/pages/BaoCaoNhapKhoTheoMay.jsx
...
import { getBaoCaoNhapKhoTheoMay } from '../../api/baoCaoNhapKhoApi';
...
const BaoCaoNhapKhoTheoMay = () => {
    ...
    const fetchStores = async () => {
      try {
        ...
        const options = res.map(item => ({
          value: item.storeId,
          label: `${item.storeId} — ${item.storeName || 'Không có tên'}`,
        }));
  ...
  useEffect(() => {
    const fetchMachines = async () => {
      try {
        ...
        const machines = await getEquipmentNames();
        const options = machines.map(item => {
          const formattedName = item.name.replace(/(\d+)$/, num => num.padStart(2, '0'));
          return { value: item.spare_5 || item.id, label: formattedName };
        });
        ...
        const fallback = Array.from({ length: 17 }, (_, i) => ({
          value: (i + 1).toString(), label: `Máy ${i + 1}`,
        }));
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      const res = await getBaoCaoNhapKhoTheoMay(form);
      if (res.success) {
        setData(res.data || []);
  ...
  const exportToExcel = () => {
    ...
    // Chuẩn bị dữ liệu cho Excel (Header tiếng Việt có dấu cho chuyên nghiệp)
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "BaoCaoNhapKho");

    // Tự động căn chỉnh chiều rộng cột (tương đối)
    const wscols = [
      { wch: 5 },
      { wch: 20 },
      { wch: 35 },
    ...
    // Xuất file
    const fileName = `BaoCao_NhapKho_TheoMay_${now.toISOString().slice(0,10)}.xlsx`;
  ...
  const exportToPDF = () => {
    ...
    tableBody.push([
      { content: 'TỔNG CỘNG', colSpan: 5, styles: { halign: 'center', fontStyle: 'normal' } },
    ...
    autoTable(doc, {
      startY: 80,
      head: [['STT', 'Quy Cách', 'Ký hiệu\nmã QC', 'Máy TH', 'ĐVT', 'Số Lượng', 'GHI CHÚ']],
      body: tableBody,
      theme: 'grid',
    ...
    doc.save(`BaoCao_NhapKho_TheoMay_${now.toISOString().slice(0,10)}.pdf`);
  };
  ...
  const totalSL  = data.reduce((s, row) => s + (Number(row[3]) || 0), 0);
  const soMay    = [...new Set(data.map(r => r[4]).filter(Boolean))].length;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#e0eaf4', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
      ...
      {data.length > 0 && (
        <div className="mes-kpi-row mes-fade">
          ...
          <KpiCard label="Số Máy"       value={soMay.toString()}                 color="#166534" />
        </div>
      )}
        ...
        {data.length > 0 ? (
          <>
            <div className="mes-table-header">
              <span>
                Hiển thị <strong style={{ color: '#1565C0' }}>{data.length}</strong> bản ghi
                · Tổng SL: <strong style={{ color: '#1a3a5c' }}>{totalSL.toLocaleString('vi-VN')}</strong>
                · Số máy: <strong style={{ color: '#166534' }}>{soMay}</strong>
              </span>
              ...
              <table className="mes-table">
                <thead>
                  <tr>
                    ...
                    <th className="c" style={{ width: 80 }}>Máy</th>
                ...
                <tbody>
                  {data.map((row, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="c">
                        <span className="mes-badge mes-badge-blue">{row[4] || '—'}</span>
                      </td>
                      <td className="c">
                        <span className="mes-badge mes-badge-green">{row[6] || 'Chiếc'}</span>
                      </td>
...
export default BaoCaoNhapKhoTheoMay;