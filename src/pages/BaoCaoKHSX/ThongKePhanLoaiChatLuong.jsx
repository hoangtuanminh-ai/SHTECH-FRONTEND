...
import { getThongKeLopKsclPhanLoaiChatLuong, getThongKeLopKsclPhanLoaiChatLuongKeHoachThang, getViewStoreListOrks, getDmMayXRay, getListToSx } from '../../api/kcsWebApi';
...
const ThongKePhanLoaiChatLuong = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      let res = [];
      if (isKeHoachThang) {
        res = await getThongKeLopKsclPhanLoaiChatLuongKeHoachThang({ ...payload, monthlyPlan: Number(monthlyPlan) });
      } else {
        res = await getThongKeLopKsclPhanLoaiChatLuong(payload);
      }
  ...
  const exportToPDF = () => {
    ...
    // 6. Định nghĩa và định dạng dữ liệu cho bảng (có ẩn tên quy cách trùng lặp)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#e0eaf4', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
      ...
      <div className="mes-statusbar">
        ...
        <span>CÔNG TY CP CAO SU ĐÀ NẴNG · THỐNG KÊ LỐP KSCL PHÂN LOẠI CHẤT LƯỢNG</span>
      </div>
    </div>
  );
};

export default ThongKePhanLoaiChatLuong;
