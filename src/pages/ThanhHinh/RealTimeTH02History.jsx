...
const RealTimeTH02History = ({ equipmentId, onClose }) => {
  ...
  // Hàm xử lý xuất dữ liệu bảng lịch sử thông số hoạt động ra file Excel
  const handleExportExcel = () => {
    console.log(`>>> [RealTimeTH02History] Người dùng bấm Xuất Excel - Máy: ${equipmentId}, Số bản ghi: ${historyList.length}`);
    const timeStamp = new Date().toISOString().replace(/[-:T.]/g, '').slice(0, 14);
    exportTableToExcel({
      data: historyList,
      columns: dynamicFields,
      fileName: `LichSu_ThongSo_HoatDong_${equipmentId || 'TH02'}_${timeStamp}`,
