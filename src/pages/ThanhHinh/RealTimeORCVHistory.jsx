...
const RealTimeORCVHistory = ({ equipmentId, onClose }) => {
  ...
  // Hàm xử lý xuất dữ liệu bảng lịch sử thông số hoạt động máy Cắt Vải ra file Excel
  const handleExportExcel = () => {
    console.log(`>>> [RealTimeORCVHistory] Người dùng bấm Xuất Excel - Máy: ${equipmentId}, Số bản ghi: ${historyList.length}`);
    const timeStamp = new Date().toISOString().replace(/[-:T.]/g, '').slice(0, 14);
    exportTableToExcel({
      data: historyList,
      columns: columns,
      fileName: `LichSu_ThongSo_HoatDong_${equipmentId || 'ORCV'}_${timeStamp}`,
      sheetName: 'ThongSoRealTimeCV',
