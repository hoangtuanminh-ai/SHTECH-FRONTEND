...
import { getChartRealTimeTH09History } from '../../api/thanhhinhApi';
...
const RealTimeTH09History = ({ equipmentId, onClose }) => {
  ...
  const formatDateTimeTo14Char = (dateTimeStr, isEnd = false) => {
    ...
    console.log(`>>> [RealTimeTH09History] formatDateTimeTo14Char: Input: ${dateTimeStr}, isEnd: ${isEnd} => Output: ${result}`);
    return result;
  };
  ...
  const fetchHistory = async (pageIndex = 0) => {
    ...
    try {
      ...
      console.log(">>> [FETCH REALTIME TH09 HISTORY POPUP] gửi params:", params);
      const res = await getChartRealTimeTH09History(params);
      if (res && res.content) {
      ...
      console.error(error);
      toast.error('Lỗi khi tải lịch sử RealTime TH09');
    } finally {
      setLoading(false);
    }
  };

  // Hàm xử lý xuất dữ liệu bảng lịch sử thông số hoạt động máy TH09+ ra file Excel
  const handleExportExcel = () => {
    console.log(`>>> [RealTimeTH09History] Người dùng bấm Xuất Excel - Máy: ${equipmentId}, Số bản ghi: ${historyList.length}`);
    const timeStamp = new Date().toISOString().replace(/[-:T.]/g, '').slice(0, 14);
    exportTableToExcel({
      data: historyList,
      columns: dynamicFields,
      fileName: `LichSu_ThongSo_HoatDong_${equipmentId || 'TH09'}_${timeStamp}`,
      sheetName: 'ThongSoHoatDong',
      title: `LỊCH SỬ THÔNG SỐ HOẠT ĐỘNG (REALTIME) — MÁY ${equipmentId}`,
      metadata: {
        'Mã máy': equipmentId,
  ...
  const renderValue = (val, key) => {
    ...
    if (typeof val === 'string' && val.includes('T') && /^\d{4}-\d{2}-\d{2}T/.test(val)) {
      ...
      console.log(`>>> [RealTimeTH09History] renderValue (date format with ms): ${val} -> ${result}`);
      return result;
    }
    return String(val);
  };
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                console.log(">>> [RealTimeTH09History] fromDate hour change:", newDateTime);
            ...
            <select
              ...
              onChange={(e) => {
                ...
                console.log(">>> [RealTimeTH09History] fromDate minute change:", newDateTime);
                setFromDate(newDateTime);
              }}
              style={{ height: '26px', padding: '2px 5px', fontSize: '11px', border: '1px solid #cbd5e1', borderRadius: '3px' }}
            >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                const newDateTime = `${date}T${e.target.value}:${min}`;
                console.log(">>> [RealTimeTH09History] toDate hour change:", newDateTime);
            ...
            <select
              ...
              onChange={(e) => {
                ...
                const newDateTime = `${date}T${hour}:${e.target.value}`;
                console.log(">>> [RealTimeTH09History] toDate minute change:", newDateTime);
                setToDate(newDateTime);
              }}
              style={{ height: '26px', padding: '2px 5px', fontSize: '11px', border: '1px solid #cbd5e1', borderRadius: '3px' }}
            >
          ...
          <button 
            ...
            title="Xuất bảng lịch sử thông số hoạt động ra file Excel"
          >
...
export default RealTimeTH09History;
