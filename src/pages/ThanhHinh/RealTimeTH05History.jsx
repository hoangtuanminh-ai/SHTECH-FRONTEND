...
import { getChartRealTimeTH05History } from '../../api/thanhhinhApi';
...
const RealTimeTH05History = ({ equipmentId, onClose }) => {
  ...
  const getDynamicFields = (record) => {
    ...
    // Danh sách các cột thông số thiết bị TH05 Việt hóa theo đúng yêu cầu
    const displayFields = [
  ...
  const formatDateTimeTo14Char = (dateTimeStr, isEnd = false) => {
    ...
    console.log(`>>> [RealTimeTH05History] formatDateTimeTo14Char: Input: ${dateTimeStr}, isEnd: ${isEnd} => Output: ${result}`);
    return result;
  };
  ...
  const fetchHistory = async (pageIndex = 0) => {
    ...
    try {
      ...
      console.log(">>> [FETCH REALTIME TH05 HISTORY POPUP] gửi params:", params);
      const res = await getChartRealTimeTH05History(params);
      if (res && res.content) {
      ...
      console.error(error);
      toast.error('Lỗi khi tải lịch sử RealTime TH05');
    } finally {
      setLoading(false);
    }
  };

  // Hàm xử lý xuất dữ liệu bảng lịch sử thông số hoạt động máy TH05 ra file Excel
  const handleExportExcel = () => {
    console.log(`>>> [RealTimeTH05History] Người dùng bấm Xuất Excel - Máy: ${equipmentId}, Số bản ghi: ${historyList.length}`);
    const timeStamp = new Date().toISOString().replace(/[-:T.]/g, '').slice(0, 14);
    exportTableToExcel({
      data: historyList,
      columns: dynamicFields,
      fileName: `LichSu_ThongSo_HoatDong_${equipmentId || 'TH05'}_${timeStamp}`,
...
export default RealTimeTH05History;
