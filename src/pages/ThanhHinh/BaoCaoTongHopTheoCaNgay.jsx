// src/pages/ThanhHinh/BaoCaoTongHopTheoCaNgay.jsx
import React, { useState, useEffect } from 'react';
import { toast } from 'react-toastify';
import * as XLSX from 'xlsx';
import { getTongHopKhongTheoMay, getCatVaiStores, getCatVaiEquipments } from '../../api/catVaiApi';
...
function BaoCaoTongHopTheoCaNgay() {
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      console.log(`Searching BaoCaoTongHopTheoCaNgay: idKehoach=${idKehoach}, startShift=${startShift}, endShift=${endShift}, storeId=${storeId}, maMay=${maMay}`);
      const res = await getTongHopKhongTheoMay(idKehoach || '%', Number(startShift), Number(endShift), storeId, maMay || '%');
      if (res && res.success) {
        setData(res.data || []);
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Tổng Hợp Theo Ca Ngày");
    XLSX.writeFile(wb, "BaoCao_TongHop_TheoCaNgay.xlsx");
        ...
        <div className="report-modal-overlay">
          <div className="report-modal-container">
            <div className="report-modal-header">
              <h3>Xem trước Báo cáo Tổng hợp Bán chế phẩm Cắt vải Sản xuất</h3>
              <button 
              ...
              <div className="print-page">
                <div className="print-header">
                  <div className="print-header-center">
                    XÍ NGHIỆP LỐP RADIAL<br/>
                    TỔNG HỢP SẢN XUẤT CÔNG ĐOẠN CẮT VẢI
                  </div>
                </div>

                <div className="print-title">
                  <p style={{ textAlign: 'center', fontWeight: 'bold', fontSize: '13px', margin: 0 }}>
                    Ca {startShiftVal}, {formatVietnameseDate(startShift)}
                  </p>
                </div>
                  ...
                  <tbody>
                    {data.map((row, idx) => (
                      <tr key={idx}>
                        ...
                        <td>{row.tenBtp || row.Ten_BTP || '—'}</td>
                        <td>{row.kyHieuBtp || row.KyHieu_BTP || '—'}</td>
                        <td>{row.loaiBtp || row.Loai_BTP || '—'}</td>
...
export default BaoCaoTongHopTheoCaNgay;
