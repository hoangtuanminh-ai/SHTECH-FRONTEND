...
const SettingTH05History = ({ equipmentId, onClose }) => {
  ...
  // Hàm xử lý xuất dữ liệu bảng lịch sử thông số cài đặt máy TH05 ra file Excel
  const handleExportExcel = () => {
    console.log(`>>> [SettingTH05History] Người dùng bấm Xuất Excel - Máy: ${equipmentId}, Số bản ghi: ${historyList.length}`);
    const timeStamp = new Date().toISOString().replace(/[-:T.]/g, '').slice(0, 14);
    exportTableToExcel({
      data: historyList,
      columns: dynamicFields,
      fileName: `LichSu_ThongSo_CaiDat_${equipmentId || 'TH05'}_${timeStamp}`,
