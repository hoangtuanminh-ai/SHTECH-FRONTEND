// src/pages/ThanhHinh/BaoCaoTongHopCatVai.jsx
...
const css = `
  ...
  .report-modal-container {
    ...
    width: 90%; max-width: 950px; max-height: 90vh;
  ...
  /* Định dạng mẫu báo cáo tổng hợp in ấn */
  .print-page {
    background: #fff;
    width: 100%;
    margin: 0 auto;
  ...
  .print-title h2 { margin: 0 0 5px 0; font-size: 16px; font-weight: bold; text-transform: uppercase; }
  .print-title p { margin: 0; font-size: 12px; font-style: italic; }
  .print-table { width: 100%; border-collapse: collapse; margin-top: 10px; font-size: 11px; }
  .print-table th { border: 1px solid #000; padding: 5px 3px; font-weight: bold; text-align: center; background: #fff; }
...
function BaoCaoTongHopCatVai() {
  ...
  const handleSearch = async () => {
    ...
    if (!startShift || !endShift) {
      toast.error("Vui lòng nhập Khoảng ca bắt đầu và kết thúc");
      return;
    }

    setLoading(true);
    try {
      console.log(`Searching BaoCaoTheoXeRange: startShift=${startShift}, endShift=${endShift}, storeId=${storeId}, maMay=${maMay}`);
      const res = await getBaoCaoTheoXeRange(startShift, endShift, storeId, maMay || '%');
      if (res && res.success) {
        setData(res.data || []);
        toast.success(res.message || "Tải danh sách lốp thành công");
      } else {
        toast.error(res?.message || "Lỗi tải danh sách lốp");
      }
    } catch (err) {
  ...
  const handleOpenReport = async () => {
    if (!storeId || !startShift || !endShift) {
      toast.error("Vui lòng nhập đầy đủ Kho, Ca bắt đầu và Ca kết thúc");
      return;
    }
    ...
    try {
      ...
      console.log(`Fetching getTongHopIDKeHoach: idKehoach=${queryIdKehoach}, startShift=${startShift}, endShift=${endShift}, storeId=${storeId}, maMay=${maMay}`);
      const res = await getTongHopIDKeHoach(queryIdKehoach, Number(startShift), Number(endShift), storeId, maMay || '%');
      if (res && res.success) {
  ...
  const exportExcelReport = () => {
    if (!reportData.length) {
      toast.warn("Không có dữ liệu báo cáo để xuất");
      return;
    }
    const ws = XLSX.utils.json_to_sheet(reportData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Tổng hợp Kế hoạch");
    XLSX.writeFile(wb, `BaoCao_TongHop_KHSX_${idKehoach || "Empty"}.xlsx`);
    toast.success("Xuất file Excel thành công");
  };

  const exportExcelRaw = () => {
    if (!data.length) {
    ...
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Danh sách lốp");
    XLSX.writeFile(wb, "DanhSachLopNhapKhoCatVai.xlsx");
    toast.success("Xuất danh sách lốp thành công");
  };

  // Tính tổng kết cho báo cáo trong popup
  const totalKeHoach = reportData.reduce((acc, cur) => acc + (Number(cur.SoLuong_KH) || 0), 0);
  const totalDieuChinh = reportData.reduce((acc, cur) => acc + (Number(cur.SoLuong_KH_DieuChinh) || 0), 0);
  const totalDaSanXuat = reportData.reduce((acc, cur) => acc + (Number(cur.SoLuong_SX) || 0), 0);
  const totalLech = reportData.reduce((acc, cur) => acc + (Number(cur.SoLuong_Thieu) || 0), 0);
  ...
  // Tự động ghép idKehoach = [storeId].[idDate_yyyyMMdd][idShiftVal] hoàn toàn độc lập khi thay đổi
  useEffect(() => {
    if (storeId && idDate) {
  ...
  return (
    <div className="mes-container">
      ...
      <div className="mes-toolbar">
        ...
        <button className="mes-tb-btn" onClick={handleOpenReport}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
          <span>Xuất Báo Cáo</span>
        </button>
        <button className="mes-tb-btn" onClick={exportExcelRaw}>
      ...
      <div className="mes-table-area">
        <div className="mes-table-wrap">
          <table className="mes-table">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan="10" style={{ textAlign: "center", padding: "20px" }}>Đang tải danh sách lốp...</td></tr>
              ) : data.length === 0 ? (
                <tr><td colSpan="10" style={{ textAlign: "center", padding: "20px" }}>Không có dữ liệu</td></tr>
              ) : (
                data.map((row, idx) => (
                  <tr key={idx}>
        ...
        <div className="report-modal-overlay">
          <div className="report-modal-container">
            <div className="report-modal-header">
              <h3>Xem trước Báo cáo Tổng hợp Kế hoạch Sản xuất BCP Cắt Vải</h3>
              <button 
            ...
            <div className="report-modal-body">
              {reportLoading ? (
                <div style={{ textAlign: 'center', padding: '40px', fontFamily: 'Segoe UI, sans-serif' }}>
                  Đang lập báo cáo tổng hợp...
                </div>
              ) : (
                <div className="print-page">
                  ...
                  <div className="print-header">
                    <div className="print-header-left">
                      ...
                      XN LỐP RADIAL - BP.CẮT VẢI
                    </div>
                    <div className="print-header-right">
                      {idKehoach ? idKehoach : 'OR.C.???'}
                    </div>
                  </div>

                  {/* Tên báo cáo */}
                  <div className="print-title">
                    <h2>TỔNG HỢP KẾ HOẠCH SẢN XUẤT<br/>BCP CẮT VẢI</h2>
                    <p style={{ marginTop: 5 }}>
                      Từ ca {startShiftVal} ngày {startDate ? startDate.split("-").reverse().join("/") : "—"} đến ca {endShiftVal} ngày {endDate ? endDate.split("-").reverse().join("/") : "—"}
                    </p>
                  </div>
                  ...
                  <table className="print-table">
                    <thead>
                      <tr>
                        ...
                        <th>QUY CÁCH</th>
                        <th>KÝ HIỆU</th>
                        <th style={{ width: '50px' }}>MÃ MÁY</th>
                        <th style={{ width: '70px' }}>KẾ HOẠCH</th>
                        <th style={{ width: '75px' }}>ĐIỀU CHỈNH</th>
                        <th style={{ width: '80px' }}>ĐÃ SẢN XUẤT</th>
                        <th style={{ width: '80px' }}>LỆCH KẾ HOẠCH</th>
                        <th>GHI CHÚ</th>
                      </tr>
                    </thead>
                    <tbody>
                      {reportData.length === 0 ? (
                        <tr>
                          <td colSpan="9" style={{ textAlign: 'center', padding: '15px' }}>Không tìm thấy dữ liệu kế hoạch phù hợp</td>
                        </tr>
                      ) : (
                        reportData.map((row, idx) => (
                          <tr key={idx}>
                            <td style={{ textAlign: 'center' }}>{idx + 1}</td>
                            <td>{row.Ten_BTP || row.tenBtp || '—'}</td>
                            <td>{row.KyHieu_BTP || row.kyHieuBtp || '—'}</td>
                            <td style={{ textAlign: 'center' }}>{row.MaMay || row.maMay || '—'}</td>
                            <td style={{ textAlign: 'right' }}>{(row.SoLuong_KH || 0).toLocaleString('vi-VN')}</td>
                            <td style={{ textAlign: 'right' }}>{(row.SoLuong_KH_DieuChinh || 0).toLocaleString('vi-VN')}</td>
                            <td style={{ textAlign: 'right', fontWeight: 'bold' }}>{(row.SoLuong_SX || 0).toLocaleString('vi-VN')}</td>
                            <td style={{ textAlign: 'right', color: '#b91c1c' }}>{(row.SoLuong_Thieu || 0).toLocaleString('vi-VN')}</td>
                      ...
                      {reportData.length > 0 && (
                        <tr style={{ fontWeight: 'bold' }}>
                          ...
                          <td></td>
                          <td></td>
                          <td style={{ textAlign: 'right' }}>{totalKeHoach.toLocaleString('vi-VN')}</td>
                          <td style={{ textAlign: 'right' }}>{totalDieuChinh.toLocaleString('vi-VN')}</td>
                          <td style={{ textAlign: 'right' }}>{totalDaSanXuat.toLocaleString('vi-VN')}</td>
                          <td style={{ textAlign: 'right' }}>{totalLech.toLocaleString('vi-VN')}</td>
                          <td></td>
                        </tr>
                      )}
                    </tbody>
...
export default BaoCaoTongHopCatVai;
