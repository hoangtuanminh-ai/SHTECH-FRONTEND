...
const ThanhHinhChangeHistory = ({ equipmentId, onClose }) => {
  ...
  // Hàm xử lý xuất dữ liệu lịch sử thay đổi cấu hình máy Thành Hình ra file Excel
  const handleExportExcel = () => {
    console.log(`>>> [ThanhHinhChangeHistory] Người dùng bấm Xuất Excel - Máy: ${equipmentId}, Số bản ghi: ${historyList.length}`);
    const timeStamp = new Date().toISOString().replace(/[-:T.]/g, '').slice(0, 14);
    exportTableToExcel({
      data: historyList,
      columns: dynamicFields,
      fileName: `LichSu_ThayDoi_ThongSo_${equipmentId || 'TH'}_${timeStamp}`,
      sheetName: 'LichSuThayDoi',
      title: `LỊCH SỬ THAY ĐỔI PARAMETER SETTING — MÁY ${equipmentId}`,
      metadata: {
        'Mã máy': equipmentId,
